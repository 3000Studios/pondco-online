...
Every production-relevant change must deploy only through the canonical Cloudflare project with Wrangler. Verify the live canonical domain afterwards on desktop and mobile: routes, images, console/network errors, layout shifts, and horizontal scrolling. Log the commit, deployment, domain, and result. Never use another host, invent deployment success, or expose secrets.
